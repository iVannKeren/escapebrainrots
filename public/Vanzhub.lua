-- Brainrot SC Loaded - Mobile Ready - Fuck the servers! by VannZZ
...
-- Mobile Buttons (pojok kanan atas, touch friendly, selalu muncul pas GUI aktif)
...
-- Floating icon setelah minimize (draggable)
...
-- Auto tampilkan tombol mobile kalau TouchEnabled
if UserInputService.TouchEnabled then
    toggleGUI(true)  -- auto buka GUI di mobile
end
    ...
    spawn(function()
        while toggles.AutoSpeed do
            pcall(function()
                ...
                -- Kalau ada remote speed: ReplicatedStorage.SpeedEvent:FireServer(10)
            end)
            wait(1)
        end
    end)
...
TeleTab:NewButton("TP to Shop", "", function()
    for _, v in pairs(Workspace:GetDescendants()) do
        if v:IsA("BasePart") and v.Name:lower():find("shop") then
            pcall(function() RootPart.CFrame = v.CFrame + Vector3.new(0,5,0) end)
            break
        end
    end
...
AutoTab:NewToggle("Auto Rebirth / Buy Upgrades", "Rebirth & upgrade otomatis", function(state)
    ...
    spawn(function()
        while toggles.AutoRebirth do
            pcall(function()
                -- Ganti nama remote sesuai game asli (cek pakai Dex Explorer)
                ...
                if ReplicatedStorage:FindFirstChild("Purchase") then
                    ReplicatedStorage.Purchase:FireServer("Speed")  -- contoh, sesuaikan
                end
            end)
            wait(2)
        end
